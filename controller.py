...
# Create a dictionary that maps each course name to a list of all layout names for that course
...
def index():
    ...
    course_names.sort() # Sort the course names alphabetically
    player_names.sort() # Sort the player names alphabetically
    # If the user has submitted the form, retrieve the selected player, course, and layout names
    if request.method == 'POST':
        selected_player = request.form['player']
        selected_course = request.form['course']

        # Filter the DataFrame to only include rounds played by the selected player on the selected course and layout
        if selected_player == 'All Players':
            # If the user selected "All Players," only filter by the selected course and layout
        ...
        # Get the top 10 scores for the selected player on the selected course and layout
        top10_scores = filtered_df.nsmallest(10, 'tulos')

        # Render the template with the selected player, course, and layout names and the top 10 scores
